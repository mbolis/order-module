...
function om_send_client_order_report($data, $ext = 'xlsx') {
	...
	$address = "Tel.: " . $order['telefono_cliente'];
	if ($order['id_gas']) {
		$recipient = $recipient . "\nc/o " . $order['nome_contatto_gas'] . " (GAS: " . $order['nome_gas'] . ')';
	} else {
	...
	for ($i = 0; $i < count($products); $i++) {
		...
		$qty_text = $product['quantita_testo'];
		...
		$sheet->setCellValue("B$rown", $description)
					...
					->setCellValue("D$rown", $qty > 0 ? $qty : $qty_text)
...
Tel.: ${order['telefono_gas']}");
	...
	foreach ($products as $id => $product) {
		...
		foreach ($client_orders as $order) {
			...
			$qtys = $order['righe_ordine'][$id];
			$qty = $qtys['quantita'];
			$qty = $qty ? $qty : 0;
			$qty_txt = $qtys['quantita_testo'];
			$sheet->setCellValue("$qty_col$row", $qty_txt ? $qty_txt : (float) $qty)
				->getStyle("$qty_col$row")->getNumberFormat()->setFormatCode("0.### \"$unit_abbr\";;;@");

			$sheet->setCellValue("$price_col$row", "=IF(ISNUMBER($qty_col$row),\$B$row*$qty_col$row,0)")
		...
		$sheet->getCell("C$row")->setValueExplicit("=SUM(IF(ISEVEN(COLUMN(D$row:$max_col$row)),D$row:$max_col$row,0))", PHPExcel_Cell_DataType::TYPE_FORMULA_ARRAY);
	...
	foreach ($gas_map as $gas_range) {
		...
		for ($row = $first_prod_row; $row <= $last_prod_row; $row++) {
			$gas_sheet->getCell("C$row")->setValueExplicit("=SUM(IF(ISEVEN(COLUMN(D$row:${gas_range['end']}$row)),D$row:${gas_range['end']}$row,0))", PHPExcel_Cell_DataType::TYPE_FORMULA_ARRAY);
		}
		...
		for ($row=1; $row<=$notes_row; $row++) {
			...
			for ($src_col=$gas_range['start'], $dest_col=$min_col; $src_col!=$gas_range['end']; $src_col++, $dest_col++) {
				...
				if ($src_col == $gas_range['end']) break;
			}
		}
